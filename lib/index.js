...
import Interceptor from '@pureproxy/utils/lib/interceptor.js'
import { HTTPParser, methods } from '@pureproxy/pureproxy/lib/parser.js'
...
export class HttpTrace extends MitmProxy {
  ...
  saveToFile(filename, data, subject) {
    subject;
  ...
  async serializeRequest(request) {
    ...
    const contentEncoding = this.getHttpHeader(
      request.headers,
      'content-encoding'
    )
  ...
  async serializeResponse(response) {
    console.log(
      ...
      1,
      response.id,
      response.subject.hostname,
      response.subject.port,
  ...
  async wrapClientForObservableStreaming(client, subject) {
    const id = Date.now()
